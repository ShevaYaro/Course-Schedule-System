...
class BSTMap:
    ...
    def _height_recursive(self, node):
        # Base case: if the tree/subtree is empty, return -1
        if node is None:
            return -1
        
        # Recursive step: find the height of left and right subtrees
        left_height = self._height_recursive(node.left)
        right_height = self._height_recursive(node.right)
        
        # The height of this node is 1 plus the maximum of its children's heights
...
class AVLTreeMap:
    ...
    # ------------------------ HEIGHT ------------------------
    def height(self):
        """Return the true computed height of the AVL tree."""
        return self._height_recursive(self._root)

    def _height_recursive(self, node):
        # Base case: empty node is -1
        if node is None:
            return -1
            
        # Recursive step: calculate left and right paths
        left_height = self._height_recursive(node.left)
        right_height = self._height_recursive(node.right)
        
        # Return 1 + the longest path
        return 1 + max(left_height, right_height)
    # ---------------------------------------------------------
